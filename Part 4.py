...
print("Part 1:")
while True:
    while True:                                 #Loop until correct student ID is entered
        student_ID = input("Enter student ID: ")
        if student_ID[0] == "w" and len(student_ID) == 8 and (student_ID[1:8]).isdigit() == True:   #Starts with w, check whether there are 8 elements, from index 1 to 8 check whether all are digits
            keys=dict1.keys()                   #keys variable to store student_ID as a key
    ...
    if total_credits == 120:
        ...
        if pass_credits == 120:
            ...
            dict1[student_ID] = ["Progress - ", pass_credits, defer_credits, fail_credits]
            ...
            dict1[student_ID] = ["Progress (module trailer) - ", pass_credits, defer_credits, fail_credits]
            ...
            dict1[student_ID] = ["Exclude - ", pass_credits, defer_credits, fail_credits]
            ...
            dict1[student_ID] = ["Module retriever - ", pass_credits, defer_credits, fail_credits]
...
#TASK 4 - Dictionary
print("\nPart 4:")
print(str(dict1).replace("{",'').replace("}",'').replace("[",'').replace("]",'').replace("'",''))           #Print dictionary
